...
def test_parse_json_from_text_json_block():
    from pictova.engine.vision_chain import _parse_json_from_text
    text = '```json\n{"alt": "test", "title": "t"}\n```'
...
def test_find_bin_uses_shutil_first(tmp_path):
    from pictova.engine.vision_chain import _find_bin
    with patch("pictova.engine.vision_chain.shutil.which", return_value="/usr/bin/somebin"):
...
def test_find_bin_fallback_to_npm_path(tmp_path):
    ...
    with patch("pictova.engine.vision_chain.shutil.which", return_value=None), \
         patch("pictova.engine.vision_chain.Path.home", return_value=tmp_path):
...
def test_has_any_vision_source_with_claude_bin():
    ...
    with patch.dict("os.environ", {"GEMINI_API_KEY": ""}), \
         patch("pictova.engine.vision_chain._codex_check_login", return_value=False), \
         patch("pictova.engine.vision_chain._find_bin", side_effect=lambda n: "/bin/claude" if n == "claude" else None):
        assert has_any_vision_source() is True
...
def test_has_any_vision_source_false_when_nothing():
    from pictova.engine.vision_chain import has_any_vision_source
    with patch.dict("os.environ", {"GEMINI_API_KEY": ""}), \
         patch("pictova.engine.vision_chain.urllib.request.urlopen", side_effect=OSError), \
         patch("pictova.engine.vision_chain._codex_check_login", return_value=False), \
         patch("pictova.engine.vision_chain._find_bin", return_value=None):
...
def test_vision_chain_raises_when_all_fail(tmp_path):
    ...
    with patch("pictova.engine.vision_chain._analyze_gemini_flash", side_effect=RuntimeError("no key")), \
         patch("pictova.engine.vision_chain._analyze_codex", side_effect=RuntimeError("no login")), \
         patch("pictova.engine.vision_chain._analyze_claude_cli", side_effect=RuntimeError("no claude")):
...
def test_vision_chain_returns_first_success(tmp_path):
    ...
    with patch("pictova.engine.vision_chain._analyze_gemini_flash", return_value=dict(expected)), \
         patch("pictova.engine.vision_chain._analyze_codex") as mock_codex, \
         patch("pictova.engine.vision_chain._analyze_claude_cli") as mock_claude:
