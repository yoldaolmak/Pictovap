...
def test_has_any_vision_source_with_lm_studio_running():
    ...
    with patch.dict("os.environ", {"GEMINI_API_KEY": ""}), \
            patch("pictova.engine.vision_chain.urllib.request.urlopen"):
        assert has_any_vision_source() is True
...
def test_analyze_gemini_flash_raises_without_key():
    from pictova.engine.vision_chain import _analyze_gemini_flash
    with patch.dict("os.environ", {"GEMINI_API_KEY": ""}):
...
def test_analyze_lm_studio_raises_when_unreachable(tmp_path):
    from pictova.engine.vision_chain import _analyze_lm_studio
    with patch("pictova.engine.vision_chain.urllib.request.urlopen", side_effect=OSError("refused")):
...
def test_vision_chain_raises_when_all_providers_fail(tmp_path):
    ...
    with patch("pictova.engine.vision_chain._analyze_lm_studio", side_effect=RuntimeError("not running")), \
            patch("pictova.engine.vision_chain._analyze_gemini_flash", side_effect=RuntimeError("no key")):
...
def test_vision_chain_returns_first_success(tmp_path):
    ...
    with patch("pictova.engine.vision_chain._analyze_lm_studio", side_effect=RuntimeError("not running")), \
            patch("pictova.engine.vision_chain._analyze_gemini_flash", return_value=dict(expected)) as mock_gemini:
...
def test_vision_chain_prefers_lm_studio_when_available(tmp_path):
    ...
    with patch("pictova.engine.vision_chain._analyze_lm_studio", return_value=dict(expected)), \
            patch("pictova.engine.vision_chain._analyze_gemini_flash") as mock_gemini:
        result = analyze_image_vision_chain(str(fake_img), location_hint="test", post_context={})
