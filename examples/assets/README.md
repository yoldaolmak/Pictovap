...
This directory is referenced by the Pictovap local demo (`make demo` / `python -m pictovap.demo`).

## Mock Asset References

The demo uses mock image candidate metadata. The following filenames are referenced
in `src/pictovap/data/demo_candidates.py` as `local_path` values:
...
using the mock candidate dictionaries defined in
`src/pictovap/data/demo_candidates.py`.
...
2. Add entries to the `MOCK_CANDIDATES` list in `src/pictovap/data/demo_candidates.py` with the correct
